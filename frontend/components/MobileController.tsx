...
export default function MobileController() {
  ...
  useEffect(() => {
    ...
    console.log("Mobile attempting connection to:", socketUrl);

    const s = io(socketUrl, {
      reconnectionAttempts: 10,
      timeout: 5000,
      transports: ['websocket', 'polling'], // Critical for mobile compatibility
      extraHeaders: {
    ...
    s.on('connect_error', (err) => {
      ...
      setError(`Cannot reach server at ${socketUrl}. Make sure phone and PC are on the same Wi-Fi.`);
    });
  ...
  const sendMenuAction = (action: string) => {
    if (socket && socket.connected) {
      ...
      socket.emit('menu-action', { roomId, action });
    }
  };
  ...
  const requestPermissionsAndStart = async () => {
    try {
      if (typeof window !== 'undefined' && !window.isSecureContext) {
        ...
        if (!isLocalhost) {
          setError('Motion sensors require a secure HTTPS connection. Since you are connecting via a local IP, iOS/Android will block the sensors. Please use a tunnel like ngrok or localtunnel to test on your phone.');
          return;
        }
      }
      ...
      if (typeof window !== 'undefined' && 'DeviceMotionEvent' in window && typeof (DeviceMotionEvent as any).requestPermission === 'function') {
        ...
        if (permissionState !== 'granted') {
          setError('Motion sensor permission was denied. You must grant permission to play the motion games.');
          return;
        }
      }
      ...
      const handleMotion = (event: DeviceMotionEvent) => {
        ...
        // Use any stored orientation data if available (updated by DeviceOrientation event)
